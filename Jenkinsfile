pipeline {
    ...
    stages {
        stage('Dependencies') {
            ...
            // Triggers upstream Jenkins jobs before procceeding

            // `wait:true` blocks until each job completes successfully
            // this guarantees that dependency artifacts are up to date before the current service is built

            steps {
                build job: 'catalog', wait: true
            }
        }
        stage('Build') {
